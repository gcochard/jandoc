...
function hasFileExtension(path) {
  return p.extname(path).length>0;
}
...
function stripFileExtension(path) {
  return path.slice(0,-p.extname(path).length);
}
  ...
  var files, command, i,
      ...
      execHandler = function (err, stdout, stderr) {
          ...
          if (callback) {
            ...
            var answer = { "stderr": stderr || null, "stdout": stdout || null },
                error;

            if (err) {
              error = { "error": err, "stderr": stderr || null, "stdout": stdout || null }
            }
